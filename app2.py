...
def Talent_Zone():
    ...
    # Custom CSS for enhanced UI (keeping your existing styles + new disclaimer styles)
    st.markdown("""
    <style>
    .talent-header {
        background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    ...
    try:
        ...
        data = {
            "fields": {
                ...
                "Posted_By": st.session_state.current_user.get("Email", ""),
                "Active": True
            }
        }
    ...
    with st.form("new_announcement"):
        ...
        if st.form_submit_button("📤 Post Announcement", type="primary"):
            if title and message:
                if create_announcement(title, message):
                    st.success("✅ Announcement posted successfully!")
                    st.rerun()
                else:
                    st.error("❌ Failed to post announcement")
            else:
                st.warning("⚠️ Please fill in both title and message")
    ...
    if announcements:
        for announcement in announcements:
            ...
            <div style="background: #f8f9fa; padding: 1rem; border-radius: 10px; margin-bottom: 1rem; border-left: 4px solid #007bff;">
                ...
                <small style="color: #6c757d;">Posted on: {fields.get('Date', 'Unknown Date')}</small>
            </div>
            """, unsafe_allow_html=True)
    else:
...
# Show existing announcements
    ...
    if announcements:
        for announcement in announcements:
            fields = announcement["fields"]
            record_id = announcement["id"]  # Get the record ID
    ...
    if announcements:
        for announcement in announcements:
            ...
            <div class="announcement-card">
                <div class="announcement-title">📌 {fields.get('Title', 'No Title')}</div>
                <div class="announcement-message">{fields.get('Message', 'No Message')}</div>
                <div class="announcement-date">📅 {fields.get('Date', 'Unknown Date')}</div>
            </div>
            """, unsafe_allow_html=True)
    else:

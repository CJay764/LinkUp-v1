...
def show_login():
    ...
    if login_clicked:
        ...
        if user:
            ...
            """, unsafe_allow_html=True)
            st.rerun()
            st.info("✅ Tap the Sign In button again if you're not redirected.")
...
if not st.session_state.logged_in:
    ...
    # Get user type to determine navigation options
    user_type = st.session_state.current_user.get("User_Type", "Student")
    
    if user_type == "Business":
        # Business User Navigation - Limited options
        st.sidebar.markdown('<div class="nav-section-title">💼 Business Hub</div>', unsafe_allow_html=True)
        main_options = ["🏠 Home","💬 Chats", "🔗 Connect", "📥 Service Requests"]
        ...
        # Student User Navigation - No business options
        ...
        # Combine all options for radio (no profile_options for students)
        all_options = main_options + ["🚪 Logout"]
        page = st.sidebar.radio("Navigate to:", all_options, key="nav_student")
        
    else:  # user_type == "Both"
        # Both User Navigation - Full options
